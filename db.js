...
const addEmployee = async ({ firstname, lastname, manager_id, roleId }) => {
...
const getAllEmployees = async () => {
    const [rows, _] = await connection.execute('SELECT * FRROM employee');
    return rows;
};
...
const getDetailedRoles = async () => {
    const [rows, _] = await connection.execute('SELECT role.id, role.name, department.name as department, role.salary FROM role INNER JOIN department ON deparmnet.id = role.department_id;');
    return rows;
}
